...
public class NTripleGeneratorStep extends BaseStep implements StepInterface
{
    ...
    public NTripleGeneratorStep(StepMeta stepMeta,
            ...
            throws KettleException
    {
        ...
        String inputObject = removeSignals(getInputRowMeta().getString(row,
                meta.getInputObject(), ""));
        String inputDataType = removeSignals(getInputRowMeta().getString(row,
                meta.getInputDataType(), ""));
        String inputLangTag = removeSignals(getInputRowMeta().getString(row,
                meta.getInputLangTag(), ""));
        ...
        if (meta.getInnerIsLiteral())
        {
            ...
            {
                String objExt;
                if (!Const.isEmpty(inputDataType))
                {
                    objExt = String.format("^^<%s>", inputDataType);
                }
                else if (!Const.isEmpty(inputLangTag))
                {
                ...
                outputNTriple = String.format(LITERAL_OBJECT_TRIPLE_FORMAT,
                        inputSubject, inputPredicate, inputObject, objExt);
            }
        }
        else
        {
