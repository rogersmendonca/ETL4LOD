...
public class SparqlUpdateInsertStep extends BaseStep implements StepInterface
{
    ...
    public SparqlUpdateInsertStep(StepMeta stepMeta,
            ...
            throws KettleException
    {
        ...
        { // Executa apenas uma vez, first eh definido na superclasse
            ...
            try
            {
                data.graphUri = new String(meta.getGraphUriValue().getBytes(
                        "UTF-8"));
            ...
            try
            {
                ...
                data.sparqlUpdate = new SparqlUpdate(uri, username,
                        password);
            ...
            if (meta.getClearGraph())
            {
                try
                {
                    ...
                    int deleteResult = data.sparqlUpdate
                            .deleteGraph(data.graphUri);
                    int createResult = data.sparqlUpdate
                            .createGraph(data.graphUri);

                    if (createResult != 200)
                    {
                        throw new KettleException(
                                "Não foi possível criar grafo: "
                                        + data.graphUri);
                    }
                }
                catch (Exception e)
                {
                    throw new KettleException(
                            "Não foi possível recriar grafo.", e);
                }
            }
        }
    ...
    private void insertTripleList(SparqlUpdateInsertStepMeta meta,
            SparqlUpdateInsertStepData data, int begin, int end)
            throws KettleStepException
    {
        ...
        catch (Exception e)
        {
            ...
            if (end == begin + 1)
            { // Achou tripla invalida, n�o ha mais o que fazer
                ...
                {
                    logException(e);
                    putResultRow(data, new Response(400, "Bad Request"),
                            data.tripleList.get(begin));
                }
            }
            else
            {
    ...
    private String serializeTriples(SparqlUpdateInsertStepMeta meta,
            SparqlUpdateInsertStepData data, int begin, int end)
            throws TripleSerializationException, KettleValueException
    {
        ...
        for (int i = begin; i < end; i++)
        {
            ...
            String ntriple = rowMeta.getString(row,
                    meta.getRdfContentFieldName(), "");

            // Parseia, valida e carrega tripla
            String triple = data.stmtValidator.parseAndGetTriple(ntriple);
            if (triple == null)
                throw new TripleSerializationException("Tripla inválida: "
                        + ntriple);
    ...
    private void putResultRow(SparqlUpdateInsertStepData data,
            Response response, Object[] row) throws KettleStepException
    {
        putResultRow(data, response.getStatusCode(),
                response.getStatusMessageString(), row);
    }

    private void putResultRow(SparqlUpdateInsertStepData data,
            HttpResponse response, Object[] row) throws KettleStepException
    {
        putResultRow(data, response.getStatusLine().getStatusCode(), response
                .getStatusLine().getReasonPhrase(), row);
    }

    private void putResultRow(SparqlUpdateInsertStepData data, long code,
            String msg, Object[] row) throws KettleStepException
    {
        Object[] outputRow = row;

        outputRow = RowDataUtil.addValueData(outputRow, data.inputRowSize + 0,
                new Long(code));
        outputRow = RowDataUtil.addValueData(outputRow, data.inputRowSize + 1,
                msg);

        putRow(data.outputRowMeta, outputRow);
    }

    // private Response insertTriples(SparqlUpdateInsertStepData data, String
    // triples) throws Exception {
    //
    // // Tenta inserir triplas (at� 3 tentativas)
    // int remainingTries = 2;
    // HttpResponse result = null;
    // do {
    // result = data.sparqlUpdate.insertNTriples(data.graphUri, triples);
    // remainingTries--;
    // }
    // while (result.getStatusLine().getStatusCode() != 200 && remainingTries >
    // 0);
    //
