...
public class SparqlRunQueryStep extends BaseStep implements StepInterface {
	...
	public SparqlRunQueryStep(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta,
			Trans trans) {
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
		...
		if (row == null) {
			try {
				...
				// TODO Alterar Triple para Query nos metodos e variaveis
	...
	private void insertTripleList(SparqlRunQueryStepMeta meta, SparqlRunQueryStepData data, int begin, int end)
		...
		try {
			...
			if (end == begin + 1) { // Achou tripla invalida, n�o ha mais o que
									// fazer
	...
	private String serializeQueries(SparqlRunQueryStepMeta meta, SparqlRunQueryStepData data, int begin, int end)
		...
		for (int i = begin; i < end; i++) {
			...
			String query = rowMeta.getString(row, meta.getQueryTextContentFieldName(), "");
	...
	private void putResultRow(SparqlRunQueryStepData data, Response response, Object[] row) throws KettleStepException {
		putResultRow(data, response.getStatusCode(), response.getStatusMessageString(), row);
	}

	private void putResultRow(SparqlRunQueryStepData data, HttpResponse response, Object[] row)
			throws KettleStepException {
		putResultRow(data, response.getStatusLine().getStatusCode(), response.getStatusLine().getReasonPhrase(), row);
	}

	private void putResultRow(SparqlRunQueryStepData data, long code, String msg, Object[] row)
			throws KettleStepException {
